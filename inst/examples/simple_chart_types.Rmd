...
beeswarm_chart<-specify_base(chart_type = "swarmplot",data="tab_dat",x = "country", y ="latitude")
plot(beeswarm_chart)
...
cat_stripe_chart<-specify_base(chart_type = "heatmap",data="tab_dat",y = "site.id",color="bob",category="country")
...
The associated data with the spatial charts it always a bit tricky, because there are just so many ways that a user could bring such data in and its not possible for the system to catch them all. So, some extra attention needs to be paid for this chart type. minCombinR does do some work for you when you join spatial datasets, so we can use that to add information. At a bare minimu, if you don't just want to draw the polygons of the shape file, but you want to add some information to them, you need to make sure that the data you've loaded in actually contains useable information. A lot of data does not, and its not possible for minCombinR to fill in those gaps. 
...
spatial_chart<-specify_base(chart_type="choropleth",data="shape_dat",color="admin0Name")
plot(spatial_chart)
```
...
Here's an example where we compute some new data and 
```{r choro_2,message=FALSE,warning=FALSE,fig.height=4,fig.width=4,units="in"}
#Let's change things up a little bit, lets plot the incidence of things
#you can change the metadata, but you might wnat to store 
gin_meta<-gin_shape_dat@data$metadata

tab<-tab_dat@data[[1]] %>% group_by(location) %>% count()

tab<-left_join(gin_meta,tab,by=c("admin1Name"="location"))

spatial_chart<-specify_base(chart_type="choropleth",data="gin_shape_dat",metadata = "tab",color="n")
plot(spatial_chart)
```
...
map_chart<-specify_base("geographic map",data="tab_dat",lat = "latitude",long = "longitude",color="year")
plot(map_chart)
```
