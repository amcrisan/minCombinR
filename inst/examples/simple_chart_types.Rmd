...
It's also possible to see an individual map from only one of the shape files.
...
# Then join the metadata with the tabular data
tab <- tab_dat@data[[1]] %>% 
  group_by(location) %>% 
  count()
tab <- left_join(gin_meta, tab, by = c("admin1Name" = "location"))

# Now let's specify and plot a choropleth for the Guinea region
spatial_chart <- specify_base(chart_type = "choropleth", data = "gin_shape_dat", metadata = "tab", color = "n")
plot(spatial_chart)
```


Second, is working with co-ordinates from a tabular data file.  
The background raster image is supplied by the 'Openstreemaps' package (and the broader project). They do great work, consider supporting them if you like this feature too.
# TODO: By supporting them, do you mean "using their package?"
